...
saop_element_hash(struct saophash_hash *tb, Datum key)
{
	...
	Datum		hash = elements_tab->op->d.hashedscalararrayop.hash_fn_addr(fcinfo);
...
saop_hash_element_match(struct saophash_hash *tb, Datum key1, Datum key2)
{
	...
	Datum		result = elements_tab->op->d.hashedscalararrayop.fn_addr(fcinfo);
	...
	if (elements_tab == NULL)
	{
		...
		for (int i = 0; i < nitems; i++)
		{
			...
			else
			{
				...
				Datum		element = fetch_att(s, typbyval, typlen);
