...
verify_heapam(PG_FUNCTION_ARGS)
{
	...
	bool		on_error_stop = PG_GETARG_BOOL(1);

	if (PG_ARGISNULL(2))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("check_toast cannot be null")));
	bool		check_toast = PG_GETARG_BOOL(2);
				  ...
				  uint32 extsize)
{
	...
	int32		chunk_seq = DatumGetInt32(fastgetattr(toasttup, 2,
	...
	*expected_chunk_seq = chunk_seq + 1;
	...
	int32		expected_size = chunk_seq < last_chunk_seq ? TOAST_MAX_CHUNK_SIZE
...
check_tuple_attribute(HeapCheckContext *ctx)
{
	...
	Form_pg_attribute thisatt = TupleDescAttr(RelationGetDescr(ctx->rel), ctx->attnum);
	...
	Datum		attdatum = fetchatt(thisatt, tp + ctx->offset);
	...
	struct varlena *attr = (struct varlena *) DatumGetPointer(attdatum);
	...
	if (!ctx->tuple_could_be_pruned)
	{
		...
		ToastedAttribute *ta = (ToastedAttribute *) palloc0(sizeof(ToastedAttribute));
...
check_toasted_attribute(HeapCheckContext *ctx, ToastedAttribute *ta)
{
	...
	uint32		extsize = VARATT_EXTERNAL_GET_EXTSIZE(ta->toast_pointer);
	int32		last_chunk_seq = (extsize - 1) / TOAST_MAX_CHUNK_SIZE;
