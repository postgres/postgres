...
# uses the replacement index whether the old index has been dropped or
# has only been marked dead and no longer receives new rows.
...
# The writer, stopped before it locks ri_pk and reloads the constraint.
session s2
setup
{
    SELECT injection_points_set_local();
...
# Old index dropped before the check resumes.
