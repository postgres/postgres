...
DefineType(ParseState *pstate, List *names, List *parameters)
{
	...
	Oid			typeNamespace = QualifiedNameGetCreationNamespace(names, &typeName);
	...
	if (likeTypeEl)
	{
		...
		Type		likeType = typenameType(NULL, defGetTypeName(likeTypeEl), NULL);
		Form_pg_type likeForm = (Form_pg_type) GETSTRUCT(likeType);
...
RemoveTypeById(Oid typeOid)
{
	...
	Relation	relation = table_open(TypeRelationId, RowExclusiveLock);
...
DefineDomain(CreateDomainStmt *stmt)
{
	...
	/* Convert list of names to a name and namespace */
	Oid			domainNamespace = QualifiedNameGetCreationNamespace(stmt->domainname,
																	&domainName);

	/* Check we have creation rights in target namespace */
	AclResult	aclresult = pg_namespace_aclcheck(domainNamespace, GetUserId(),
												  ACL_CREATE);
	...
	/* passed by value */
	bool		byValue = baseType->typbyval;

	/* Required Alignment */
	char		alignment = baseType->typalign;

	/* TOAST Strategy */
	char		storage = baseType->typstorage;

	/* Storage Length */
	int16		internalLength = baseType->typlen;

	/* Type Category */
	char		category = baseType->typcategory;

	/* Array element Delimiter */
	char		delimiter = baseType->typdelim;
	...
	Datum		datum = SysCacheGetAttr(TYPEOID, typeTup,
	...
	Oid			domainArrayOid = AssignTypeArrayOid();
...
DefineEnum(CreateEnumStmt *stmt)
{
	...
	/* Convert list of names to a name and namespace */
	Oid			enumNamespace = QualifiedNameGetCreationNamespace(stmt->typeName,
																  &enumName);

	/* Check we have creation rights in target namespace */
	AclResult	aclresult = pg_namespace_aclcheck(enumNamespace, GetUserId(), ACL_CREATE);
	...
	/* Allocate OID for array type */
	Oid			enumArrayOid = AssignTypeArrayOid();
	 ...
	 * Create the array type that goes with it.
	 */
	char	   *enumArrayName = makeArrayTypeName(enumName, enumNamespace);
...
AlterEnum(AlterEnumStmt *stmt)
{
	...
	Oid			enum_type_oid = typenameTypeId(NULL, typename);

	HeapTuple	tup = SearchSysCache1(TYPEOID, ObjectIdGetDatum(enum_type_oid));
...
DefineRange(ParseState *pstate, CreateRangeStmt *stmt)
{
	...
	/* Convert list of names to a name and namespace */
	Oid			typeNamespace = QualifiedNameGetCreationNamespace(stmt->typeName,
																  &typeName);

	/* Check we have creation rights in target namespace */
	AclResult	aclresult = pg_namespace_aclcheck(typeNamespace, GetUserId(), ACL_CREATE);
	...
	/* Identify subopclass */
	Oid			rangeSubOpclass = findRangeSubOpclass(rangeSubOpclassName, rangeSubtype);
	...
	char		alignment = (subtypalign == TYPALIGN_DOUBLE) ? TYPALIGN_DOUBLE : TYPALIGN_INT;

	/* Allocate OID for array type, its multirange, and its multirange array */
	Oid			rangeArrayOid = AssignTypeArrayOid();
	Oid			multirangeOid = AssignTypeMultirangeOid();
	Oid			multirangeArrayOid = AssignTypeMultirangeArrayOid();
	...
	if (multirangeTypeName)
	{
		 ...
		 */
		Oid			old_typoid = GetSysCacheOid2(TYPENAMENSP, Anum_pg_type_oid,
	 ...
	 * Create the array type that goes with it.
	 */
	char	   *rangeArrayName = makeArrayTypeName(typeName, typeNamespace);
	...
	char	   *multirangeArrayName = makeArrayTypeName(multirangeTypeName, typeNamespace);
					  ...
					  Oid rangeOid, Oid subtype)
{
	...
	for (i = 0; i < lengthof(prosrc); i++)
	{
		...
		oidvector  *constructorArgTypesVector = buildoidvector(constructorArgTypes,
															   pronargs[i]);
						   ...
						   Oid *castFuncOid)
{
	...
	oidvector  *argtypes = buildoidvector(NULL, 0);
	...
	*castFuncOid = myself.objectId;
	...
	Datum		allParamTypes = ObjectIdGetDatum(rangeArrayOid);
	ArrayType  *allParameterTypes = construct_array(&allParamTypes,
													1, OIDOID,
													sizeof(Oid), true, 'i');
	Datum		paramModes = CharGetDatum(FUNC_PARAM_VARIADIC);
...
findTypeInputFunction(List *procname, Oid typeOid)
{
	...
	Oid			procOid2 = LookupFuncName(procname, 3, argList, true);
...
findRangeSubtypeDiffFunction(List *procname, Oid subtype)
{
	...
	Oid			procOid = LookupFuncName(procname, 2, argList, true);
...
DefineCompositeType(RangeVar *typevar, List *coldeflist)
{
	 ...
	 */
	Oid			typeNamespace = RangeVarGetAndCheckCreationNamespace(createStmt->relation,
																	 NoLock, NULL);
...
AlterDomainDefault(List *names, Node *defaultRaw)
{
	...
	Oid			domainoid = typenameTypeId(NULL, typename);

	/* Look up the domain in the type table */
	Relation	rel = table_open(TypeRelationId, RowExclusiveLock);

	HeapTuple	tup = SearchSysCacheCopy1(TYPEOID, ObjectIdGetDatum(domainoid));
...
AlterDomainNotNull(List *names, bool notNull)
{
	...
	/* Look up the domain in the type table */
	Relation	typrel = table_open(TypeRelationId, RowExclusiveLock);
	...
	if (notNull)
	{
		...
		List	   *rels = get_rels_with_domain(domainoid, ShareLock);
		...
		foreach(rt, rels)
		{
			...
			TableScanDesc scan = table_beginscan(testrel, snapshot, 0, NULL);
			TupleTableSlot *slot = table_slot_create(testrel, NULL);
						 ...
						 ObjectAddress *constrAddr)
{
	...
	char	   *ccbin = domainAddConstraint(domainoid, typTup->typnamespace,
...
AlterDomainValidateConstraint(List *names, const char *constrName)
{
	...
	/* Look up the domain in the type table */
	Relation	typrel = table_open(TypeRelationId, AccessShareLock);
	...
	foreach(rt, rels)
	{
		...
		while (table_scan_getnextslot(scan, ForwardScanDirection, slot))
		{
			...
			for (i = 0; i < rtc->natts; i++)
			{
				...
				Datum		conResult = ExecEvalExprSwitchContext(exprstate,
																  econtext,
																  &isNull);
					...
					const char *domainName, ObjectAddress *constrAddr)
{
	...
	Node	   *expr = transformExpr(pstate, constr->raw_expr, EXPR_KIND_DOMAIN_CHECK);
...
RenameType(RenameStmt *stmt)
{
	...
	HeapTuple	tup = SearchSysCacheCopy1(TYPEOID, ObjectIdGetDatum(typeOid));
...
AlterTypeOwnerInternal(Oid typeOid, Oid newOwnerId)
{
	...
	Datum		aclDatum = heap_getattr(tup,
										Anum_pg_type_typacl,
				   ...
				   Oid *oldschema)
{
	...
	Oid			oldNspOid = AlterTypeNamespace_oid(typeOid, nspOid, objsMoved);
						   ...
						   ObjectAddresses *objsMoved)
{
	...
	Oid			oldNspOid = typform->typnamespace;
	Oid			arrayOid = typform->typarray;
...
AlterType(AlterTypeStmt *stmt)
{
	...
	Relation	catalog = table_open(TypeRelationId, RowExclusiveLock);

	/* Make a TypeName so we can use standard type lookup machinery */
	TypeName   *typename = makeTypeNameFromNameList(stmt->typeName);
	HeapTuple	tup = typenameType(NULL, typename, NULL);
				 ...
				 AlterTypeRecurseParams *atparams)
{
	...
	SysScanDesc scan = systable_beginscan(catalog, InvalidOid, false,
										  NULL, 1, key);
