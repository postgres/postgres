...
getPublicationTables(Archive *fout, TableInfo tblinfo[], int numTables)
{
	...
	PublicationRelInfo *pubrinfo = pg_malloc(ntups * sizeof(PublicationRelInfo));
	j = 0;
...
dumpPublicationTable(Archive *fout, const PublicationRelInfo *pubrinfo)
{
	...
	char	   *tag = psprintf("%s %s", pubinfo->dobj.name, tbinfo->dobj.name);
...
getSubscriptions(Archive *fout)
{
	...
	int			i_subtwophasestate = PQfnumber(res, "subtwophasestate");
...
getNamespaces(Archive *fout, int *numNamespaces)
{
	...
	int			i_nspowner = PQfnumber(res, "nspowner");
	int			i_rolname = PQfnumber(res, "rolname");
