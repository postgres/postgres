...
TypeShellMake(const char *typeName, Oid typeNamespace, Oid ownerId)
{
	 ...
	 * open pg_type
	 */
	Relation	pg_type_desc = table_open(TypeRelationId, RowExclusiveLock);
	TupleDesc	tupDesc = pg_type_desc->rd_att;
...
RenameTypeInternal(Oid typeOid, const char *newTypeName, Oid typeNamespace)
{
	...
	HeapTuple	tuple = SearchSysCacheCopy1(TYPEOID, ObjectIdGetDatum(typeOid));
	...
	Oid			arrayOid = typ->typarray;

	/* Check for a conflicting type name. */
	Oid			oldTypeOid = GetSysCacheOid2(TYPENAMENSP, Anum_pg_type_oid,
...
makeArrayTypeName(const char *typeName, Oid typeNamespace)
{
	...
	char	   *arr = makeUniqueTypeName(typeName, typeNamespace, false);
	if (arr == NULL)
		ereport(ERROR,
				(errcode(ERRCODE_DUPLICATE_OBJECT),
...
moveArrayTypeName(Oid typeOid, const char *typeName, Oid typeNamespace)
{
	 ...
	 */
	char	   *newname = makeArrayTypeName(typeName, typeNamespace);
...
makeMultirangeTypeName(const char *rangeTypeName, Oid typeNamespace)
{
	 ...
	 */
	char	   *rangestr = strstr(rangeTypeName, "range");
	if (rangestr)
	{
