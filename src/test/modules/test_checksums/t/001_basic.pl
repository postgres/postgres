...
# there.  The session is later terminated by DROP DATABASE ... WITH (FORCE)
# and drops the temp table while exiting; commit that drop asynchronously,
# otherwise the exit has to flush WAL behind the traffic generated by the
# checksum workers, which on slow machines can exceed the time DROP DATABASE
# waits for the session to exit.
$bg = $node->background_psql('dropmeforce');
$bg->query_safe('SET synchronous_commit = off;');
$bg->query_safe('CREATE TEMP TABLE holdme (a int);');
...
# Write out the WAL backlog from the checksum processing so far, so that the
# sessions terminated below do not get stuck behind it when writing their
# final WAL records on slow machines.
