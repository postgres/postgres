...
contain_volatile_functions_walker(Node *node, void *context)
{
	...
	if (IsA(node, RestrictInfo))
	{
		...
		else
		{
			...
			bool		hasvolatile = contain_volatile_functions_walker((Node *) rinfo->clause,
															context);
			if (hasvolatile)
	...
	if (IsA(node, PathTarget))
	{
		...
		else
		{
			...
			bool		hasvolatile = contain_volatile_functions_walker((Node *) target->exprs,
															context);
		...
		case T_SubscriptingRef:
			{
				...
				const SubscriptRoutines *sbsroutines = getSubscriptingRoutines(sbsref->refcontainertype,
													  NULL);
				if (!sbsroutines ||
...
CommuteOpExpr(OpExpr *clause)
{
	...
	Oid			opoid = get_commutator(clause->opno);
		...
		case T_WindowFunc:
			{
				...
				List	   *args = expand_function_arguments(expr->args,
												 false, expr->wintype,
												 func_tuple);
		...
		case T_NullIfExpr:
			{
				...
				NullIfExpr *expr = (NullIfExpr *) ece_generic_processing(node);
		...
		case T_ScalarArrayOpExpr:
			{
				...
				/* Copy the node and const-simplify its arguments */
				ScalarArrayOpExpr *saop = (ScalarArrayOpExpr *) ece_generic_processing(node);
						  ...
						  Oid result_type, HeapTuple func_tuple)
{
	...
	if (include_out_arguments)
	{
		...
		Datum		proallargtypes = SysCacheGetAttr(PROCOID, func_tuple,
